...
\item[\texttt{private int recursiveMax(OthelloGame game, int iterations, int prevMin)}] This method represents the 'Max' part of the Minimax algorithm, it recursivly calls the \texttt{recursiveMin} method until it either has reach the layer indicated by \texttt{iterations} parameter or a leaf node. It might call \texttt{recursiveMax} instead if the opponent was not able to make any moves in this round.
\item[\texttt{private int recursiveMin(OthelloGame game, int iterations, int prevMax)}] This method represents the 'Mix' part of the Minimax algorithm, it recursivly calls the \texttt{recursiveMax} method until it either has reach the layer indicated by \texttt{iterations} parameter or a leaf node. It might call \texttt{recursiveMin} instead if the AI was not able to make any moves in this round.
\end{description} 
